...
namespace cs.threephase
{
	...
	*/
	 ...
	 class Center1
	{
		...
		public static int[][] ctsmv = new int[15582][];
		static int[] sym2raw = new int[15582];
		public static byte[] csprun = new byte[15582];
		...
		public static void initSym2Raw()
		{
			...
			Debug.Assert(count == 15582);
		}

		public static void createPrun()
		{
			Array.Fill<byte>(csprun, 0xFF);
			csprun[0] = 0;
			int depth = 0;
			int done = 1;
			...
			{
				bool inv = depth > 4;
				int select = inv ? 0xFF : depth;
				int check = inv ? depth : 0xFF;
				depth++;
				for (int i = 0; i < 15582; i++)
				{
					if (csprun[i] != select)
		...
		{
			Debug.WriteLine("Create Phase1 Center Move Table...");
			Center1 c = new Center1();
		...
		static int raw2sym_(int n)
		{
			int m = Array.BinarySearch<int>(sym2raw, n);
			return (m >= 0 ? m : -1);
		}
